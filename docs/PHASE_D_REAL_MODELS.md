...
| `ValueError: Cannot register duplicate model (av_hubert)` while importing AV-HuBERT | The package and flat-module import paths loaded the same AV-HuBERT model twice | Re-run `source scripts/setup_avhubert_env.sh`; it patches AV-HuBERT's `hubert.py` and `hubert_asr.py` imports to package-relative imports. |
| `ConfigAttributeError: Key 'required_seq_len_multiple' not in 'AVHubertConfig'` | Current fairseq `TransformerEncoder` expects newer wav2vec config fields than this AV-HuBERT clone defines | Re-run `source scripts/setup_avhubert_env.sh`; it adds `required_seq_len_multiple` and related fairseq compatibility fields to `AVHubertConfig`. |
| `size mismatch for decoder.layers.*.encoder_attn.*: checkpoint [1024,1024], current [1024,512]` | The seq2seq decoder was built with fairseq's old default `encoder_embed_dim=512` instead of the AV-HuBERT Large encoder's 1024-d output | Re-run `source scripts/setup_avhubert_env.sh`; it patches `hubert_asr.py` so decoder cross-attention uses the loaded encoder dimension. |
| `ConfigAttributeError: Key 'encoder_embed_dim' not in 'AVHubertSeq2SeqConfig'` | Older AV-HuBERT seq2seq config lacks the explicit decoder cross-attention encoder dimension field | Re-run `source scripts/setup_avhubert_env.sh`; it adds `AVHubertSeq2SeqConfig.encoder_embed_dim` and sets it under OmegaConf `open_dict`. |
