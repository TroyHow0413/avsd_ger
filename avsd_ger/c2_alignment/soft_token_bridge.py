...
class SoftTokenQFormerBridge(nn.Module):
    ...
    def build_inputs_embeds(
        ...
        *,
        ...
        ).input_ids.to(device).long() if post else torch.empty(
        ...
        if speaker_token_id is not None:
            positions = (pre_ids[0] == speaker_token_id).nonzero(as_tuple=True)[0]
            if positions.numel():
                identity = z_id.to(device=device, dtype=target_dtype)
                if identity.ndim == 1:
                    identity = identity.unsqueeze(0)
                bias = self.id_proj(identity)[0]
                pre_emb = pre_emb.clone()
                pre_emb[0, positions[0]] = pre_emb[0, positions[0]] + bias

        pieces = [pre_emb]
        if use_av_context:
            aligned = f_align.to(device=device, dtype=target_dtype)
            pieces.append(self.qformer(aligned))
