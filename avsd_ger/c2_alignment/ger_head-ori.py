...
class GERHead(nn.Module):
    ...
    def _render_text(
        ...
        tag = self.speaker_special_token  # always the same literal special token
        # nbest rendered as numbered candidates (lets the LLM select)
    ...
    def _inputs_embeds(
        ...
        av_emb = self.qformer(f_align.to(self.device).to(pre_emb.dtype))  # [1, n_q, D]
        ...
        if self._spk_token_id is not None:
            id_bias = self.id_proj(z_id.to(self.device).to(pre_emb.dtype))  # [D]
